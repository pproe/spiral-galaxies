...
from tensorflow.keras.layers import Input, concatenate, MaxPooling2D, Conv2D, Conv2DTranspose, Dropout
...
val_image_generator = image_data_generator.flow(x_test, batch_size=batch_size, seed=validation_seed)
...
val_mask_generator = mask_data_generator.flow(y_test, batch_size=batch_size, seed=validation_seed)
...
model.fit(train_generator, steps_per_epoch=steps_per_epoch, validation_steps=steps_per_epoch, epochs=epochs,
          verbose=1, validation_data=val_generator, shuffle=True)
