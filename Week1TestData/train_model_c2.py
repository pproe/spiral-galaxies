...
def trainModel(x_train, y_train):
    ...
    checkpoint = ModelCheckpoint(os.path.join('checkpoints', '{epoch: 02d}.h5'), monitor='val_acc', verbose=1, save_best_only=True, save_weights_only=True, mode='max')
    ...
    model.fit(x_train, y_train, epochs=NUM_EPOCHS, batch_size=BATCH_SIZE, shuffle=SHUFFLE,
            verbose=1, validation_split=0.15, callbacks=[earlystopping, checkpoint])
