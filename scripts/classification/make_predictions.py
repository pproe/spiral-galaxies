...
def loadData():
    ...
    testing_images = np.genfromtxt(
        IMAGES_PATH, dtype=np.single, max_rows=NUM_IMAGES * IMG_WIDTH * IMG_WIDTH
    )
...
def makePredictions(model, images):
    ...
    for idx, image in enumerate(images):
        ...
        plt.title(
            f"Model Prediction: {galaxy_type}, Confidence: {predictions[idx,0]:.3f}"
        )
        plt.savefig(
            os.path.join(os.getcwd(), "PDR2_i20_predictions", f"{idx}_prediction")
        )
